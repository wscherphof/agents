...
## From merkatordev/GeoWEP (component: docker/ng)

# Modern Angular App (`docker/ng/`)

> Global repo rules (commit messages, PRs, gotchas) live in the root `CLAUDE.md`.
> When `docker/ng` is opened as its own workspace root, consult the root file for
> those rules.

This code is the migration target built with modern Angular standalone
components, signals, and TypeScript.
...
- Use `@Service()` (imported from `@angular/core`) instead of
  `@Injectable({ providedIn: 'root' })` for global singleton services. Fall back
  to `@Injectable` only when deeper configuration or constructor injection is
  required.
...
- Services commonly expose signal-based state via a private writable signal and a
...
- Follow Angular naming conventions such as `name.component.ts` and
  `name.service.ts`.
- Angular Material (AM) is the default UI framework. Prefer existing AM-based
  patterns for dialogs, form fields, buttons, tabs, and table/grid-adjacent
  controls before introducing custom UI primitives.
- Modern grid services avoid circular dependencies via the `initializeGrid()`
  pattern.
...
Applies to `src/app/tabs/**`, `src/app/app.config.ts`, and
`src/app/common/locales/**`.
...
- Pointer-type handling (`mouse`/`touch`/`pen`) in the shared grid service
  drives selection/edit UX — do not alter without manual device testing.
